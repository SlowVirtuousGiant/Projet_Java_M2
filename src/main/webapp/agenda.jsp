...
<body>
	<div id="page-container">
			...
			<div class="row justify-content-center">
					...
					<c:choose>
						<c:when test="${fn:length(centres) gt 1}">
							<label class="form-control-label text-muted mt-3">Vos
								centres : </label>
							<form id="selectCentreForm" method="post"
								action="<c:url value='/agenda'/>">
				...
				<div class="container text-center mt-4">
					<ul class="legende">
						...
						<li><span class="rdv-pris-centre"></span> Rendez-vous dans
							une autre centre</li>
					</ul>
				</div>
				<div class="container">
					<div class="col-md-12">
						<div class="container-actif">
							...
							<form method="post" action="<c:url value='/agenda'/>">
								...
								<input type="hidden" name="init" value="1"> <input
									type="hidden" name="week" value="<%=currentWeek%>"> <input
									type="hidden" name="aff_id" value="<%=affectationId%>">
							</form>
							<%
								}
							%>
						...
						%>
						<div class="modal fade" id="modal<%=rdv.getId()%>" tabindex="-1"
		...
		+ " en " + patient.getNaissance()%></p>
										<p>
											<strong>Adresse : </strong><%=patient.getAdresse() + " " + patient.getVille() + " " + patient.getCode_postal()%></p>
										<p>
											<strong>Téléphone : </strong><%=patient.getTelephone()%></p>
										<%
											if (rdv.getCentre_id() != selectedCentre.getId()) {
										%>
										<p>
											<strong>Lieu du rendez-vous : <%=CentreDao.getCentreByID(rdv.getCentre_id()).getNom()%></strong>
										</p>
										<%
											}
										%>
						...
						<div class="row mt-3 mb-5">
							...
							<div class="col">
								<form id="dateForm" method="post"
									...
									<select id="weekSelect" class="form-select" name="selectedWeek">
										...
										%>
										<option value="<%=w%>"
											<%=w.equals(currentWeek) ? "selected" : ""%>>Semaine
											<%=w%></option>
										<%
											}
										%>
							...
							%>
							<div class="col">
								<form class="text-center" id="agendaActifForm" method="post"
									action="<c:url value='/agenda'/>">
									<input name="checkboxAgenda" class="form-check-input"
										type="checkbox" id="checkboxAgenda"
										<%=agendaDispo ? "checked" : ""%>> Agenda Actif <input
										type="hidden" name="majAgenda" value="maj">
								</form>
							</div>
						</div>
					</div>
